...
public class PersonActivityDetailService {
    ...
    public String findAllFromTillDateAsHtmlTable(Org org, Person person, LocalDate fromdt, LocalDate tilldt) {
        ...
        return "<table class='detail-table'>" + buff.toString() + "</table>";
    ...
    StringBuffer getTableRows(List<PersonActivityDetail> lst) {
        ...
        String person = "";
        Duration taskDuration = Duration.ZERO;
        List<PersonSpend> personsSpend = new ArrayList<>();
        ...
        for (int i = 0; i < lst.size(); i++) {
            ...
            personsSpend.add(new PersonSpend(pad.getActivity().getPerson().getFullName(), taskDuration, pad.isDone(), pad.getDetail()));
    ...
    private String getDescriptions(List<PersonSpend> personsSpend) {
    ...
    private boolean isTaskDone(List<PersonSpend> personsSpend) {
        return !personsSpend.stream().anyMatch(ps -> !ps.done);
    }

    private Duration getTotalSpend(List<PersonSpend> personsSpend) {
        return personsSpend.stream().map(ps -> ps.duration).reduce(Duration.ZERO, (a, b) -> a.plus(b));
    }

    private String getPersonsTime(List<PersonSpend> personsSpend) {
